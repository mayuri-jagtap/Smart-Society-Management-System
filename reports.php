...
if (isset($_GET['action'], $_GET['type'], $_GET['start_date'], $_GET['end_date'])) {
	...
	if($action == 'export' && $report_type != '' && $start_date != '' && $end_date != '')
	{
		if($report_type == 'Maintenance_Bill')
		{
			...
			header('Content-Disposition: attachment; filename="maintenance_bill_payment_report_for_'.$start_date.'to'.$end_date.'.csv"');
		...
		if($report_type == 'Complaint')
		{
			...
			// Output headers
			header('Content-Type: text/csv; charset=utf-8');
			header('Content-Disposition: attachment; filename="complaint_report_for_'.$start_date.'to'.$end_date.'.csv"');
		...
		if($report_type == 'Visitor')
		{
			...
			// Output headers
			header('Content-Type: text/csv; charset=utf-8');
			header('Content-Disposition: attachment; filename="visitor_report_for_'.$start_date.'to'.$end_date.'.csv"');
		...
		if($report_type == 'Payment')
		{
			...
			// Output headers
			header('Content-Type: text/csv; charset=utf-8');
			header('Content-Disposition: attachment; filename="facility_service_payment_report_for_'.$start_date.'to'.$end_date.'.csv"');
