...
if (isset($_POST['action'])) {
    ...
    $columns = array(
        ...
        'resident.name',
        'resident.ssn',
        'house.house_number', // Show house number instead of ID
        'resident.role',
        'resident.created_at'
    );
    ...
    $query = "
        SELECT resident.id, resident.name, resident.ssn, house.house_number, resident.role, resident.created_at
        FROM resident
        LEFT JOIN house ON house.id = resident.house_id
    ";

    // Total records
    $count = $pdo->query("SELECT COUNT(*) FROM resident")->fetchColumn();

    // Search filter
    $filterQuery = '';
    if (!empty($_POST['search']['value'])) {
        $search = $_POST['search']['value'];
        $filterQuery = " WHERE (resident.name LIKE '%$search%' 
                        OR resident.ssn LIKE '%$search%' 
                        OR house.house_number LIKE '%$search%' 
                        OR resident.role LIKE '%$search%')";
    }
    ...
    if ($_POST['action'] == 'fetch_maintenance') {
        ...
        // Define the base query
       $query = "
SELECT maintenance.id, maintenance.resident_id, resident.name AS resident_name, maintenance.amount, maintenance.month, maintenance.paid_date, maintenance.paid_amount, maintenance.created_at
FROM maintenance
LEFT JOIN resident ON resident.id = maintenance.resident_id
";
    ...
    if($_POST['action'] == 'fetch_available')
{
    ...
    // Get the total number of records
    $count = $pdo->query("SELECT COUNT(*) FROM $table WHERE status='available'")->fetchColumn();

    // Define the filter query
    $filterQuery = '';
    if (!empty($_POST['search']['value'])) {
        $search = $_POST['search']['value'];
        $filterQuery = " AND (house_number LIKE '%$search%' OR street_name LIKE '%$search%' OR block_number LIKE '%$search%')";
    }
    ...
    if ($_POST['action'] == 'fetch_payment') {
        ...
        // Define the base query
        $query = "
       SELECT payment.id, payment.resident_id, resident.name AS resident_name, payment.facility_id, payment.service_id, payment.amount, payment.month, payment.paid_date, payment.paid_amount, payment.created_at
FROM payment
LEFT JOIN resident ON resident.id = payment.resident_id
